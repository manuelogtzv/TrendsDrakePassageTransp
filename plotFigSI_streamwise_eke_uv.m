...
[tr_ekemr, ci_ekemr, ~, tr_np_ekemr sig_np_ekemr] = calctrends(eke_str(indx, :)'/Zmax, ...
    [data.time(1, indx)-datenum(1996, 1, 1)]/365.25, alpha, op_season);

% Calculate trends uv
[tr_uv, ci_uv, ~, tr_np_uv sig_np_uv] = calctrends(uv_str'/Zmax, ...
    [data.time-datenum(1996, 1, 1)]/365.25, alpha, op_season);
[tr_uvr, ci_uvr, ~, tr_np_uvr sig_np_uvr] = calctrends(uv_str(indx, :)'/Zmax, ...
    [data.time(1, indx)-datenum(1996, 1, 1)]/365.25, alpha, op_season);
...
r2 = plot((nanmean(stream.eke(indx, :)/Zmax)), S_m, 'linewidth', 1); 
r2.Color = rgb('Green');
% plot(nanmean(transp_lmg*1e-6), S_m, '--b', 'linewidth', 1);
axis(ax2, [0.01 0.069 min(S_m)-0.09 max(S_m)+0.09]); lim = axis;
...
text(lim(1)+0.03, mean(pf_psi), 'PF', 'FontSize', 7, 'FontName', 'SansSerif', ...
    'HorizontalAlignment', 'center');
text(lim(1)+0.03, psi_saccf, 'SACCF', 'FontSize', 7, 'FontName', 'SansSerif', ...
    'HorizontalAlignment', 'center');
xlabel(ax2, ['<EKE> [m^{2} s^{-2}]'], 'FontSize', 7, ...
    'FontName', 'SansSerif');
annotation(fig0, 'line', [0.707881445170660 0.657083333333333],   ...
    [0.116617063492064 0.116617063492064]);
...
leg.Position = [0.6462 0.6086 0.1018 0.1549];
...
p1 = plot(tr_ekemr(sig_np_ekemr==1)*1e2, S_m(sig_np_ekemr==1), 's', ...
    ...
    'markersize', 5);
b4.Color = p1.MarkerFaceColor;
b5.FaceColor = p1.MarkerFaceColor;
set(ax3, 'Box', 'on', 'TickDir', 'out', 'TickLength', [0.02 0.02], ...
    'YTick',  psiticks,  'YTickLabel', [], 'FontSize', 7, ...
    'FontName', 'SansSerif', 'XTick', [-0.2 0 0.2]);
...
col.Position(3) = 0.01;
ylabel(col, '<EKE> [m^2 s^{-2}]', 'FontSize', 7, 'FontName', 'SansSerif');
col.FontSize = 7;
col.FontName = 'SansSerif';
cbarrow(col, eke_cmap, 'up');
annotation(fig0, 'textbox', [ax1.Position(1)-0.1 sum(ax1.Position([2,4]))+0.01 ...
...
ax1.Box = 'on';
set(ax1, 'FontSize', 7, 'FontName', 'SansSerif',...
    'YTick',  psiticks,  'YTickLabel', num2str(psiticks, '%1.2f'), 'TickDir', 'out');
datetick('x');
ylabel('SSH [m]', 'FontSize', 7, 'FontName', 'SansSerif')
xlabel('');
axis(ax1, [datenum(2005, 6, 1) datenum(2019, 09, 1) min(S_m)-0.09 ...
...
r2 = plot((nanmean(stream.uv_prime(indx, :)/Zmax)), S_m, 'linewidth', 1);
r2.Color = rgb('Green');
% plot(nanmean(transp_lmg*1e-6), S_m, '--b', 'linewidth', 1);
axis(ax2, [-0.016 0.016 min(S_m)-0.09 max(S_m)+0.09]); lim = axis;
p1 = patch([ones(1, 2)*lim(1) ones(1, 2)*lim(2)], [saf_psi flip(saf_psi)], ...
    rgb('Goldenrod'), 'edgecolor', 'none', 'facealpha', 0.2); hold on;
p2 = patch([ones(1, 2)*lim(1) ones(1, 2)*lim(2)], [pf_psi flip(pf_psi)], ...
    rgb('Goldenrod'), 'edgecolor', 'none', 'facealpha', 0.2);
...
text(lim(1)+0.01, psi_saccf, 'SACCF', 'FontSize', 7, 'FontName', 'SansSerif', ...
    'HorizontalAlignment', 'center');
xlabel(ax2, '<EMF> [m^{2} s^{-2}]', 'FontSize', 7, 'FontName', 'SansSerif');
annotation(fig2, 'line', [0.707921568627450 0.657083333333333],   ...
    [0.045670037926675 0.045670037926675]);
set(ax2, 'YTick',  psiticks,  'YTickLabel', [],...
...
col.Position(4) = ax3.Position(4);
col.Position(3) = 0.01;
ylabel(col, '<EMF> [m^{2} s^{-2}]', ...
    'FontSize', 7, 'FontName', 'SansSerif');
col.FontSize = 7;
col.FontName = 'SansSerif';
caxis(ax1, [emf_cont(1) emf_cont(end)]); 
colormap(ax1, emf_cmap);
cbarrow(col, emf_cmap);

annotation(fig2, 'textbox', [ax1.Position(1)-0.1 sum(ax1.Position([2,4]))+0.01 ...
    0.0480 0.0353], 'String', 'a', 'fontsize', 7, 'fontname', 'SansSerif', ...
    'EdgeColor', 'none', 'fontweight', 'bold');
annotation(fig2, 'textbox', [ax2.Position(1)-0.02 sum(ax2.Position([2,4]))+0.01 ...
    0.0480 0.0353], 'String', 'b', 'fontsize', 7, 'fontname', 'SansSerif', ...
    'EdgeColor', 'none', 'fontweight', 'bold');
annotation(fig2, 'textbox', [ax3.Position(1)-0.025 sum(ax3.Position([2,4]))+0.01 ...
    0.0480 0.0353], 'String', 'c', 'fontsize', 7, 'fontname', 'SansSerif', ...
    'EdgeColor', 'none', 'fontweight', 'bold');

print('-dpdf', ['./FiguresPaper/fig_trendsUVPRIME_streamlines_' num2str(nday) ...
    'days.pdf'], '-r500');
