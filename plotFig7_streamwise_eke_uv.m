...
load('./Datasets/eke_uvprime_all_os38.mat');
...
load('omgeosvel_os38nb.mat'); %mean geostrophic vel
...
save(['Streamwise_binning_eke' adcp '.mat'], 'stream');
...
xlabel(ax2, ['<EKE> [m^{2} s^{-2}]'], 'FontSize', 7, ...
    'FontName', 'SansSerif');
annotation(fig0, 'line', [0.663326280586179 0.618746331441988],   ...
    [0.576954528000427 0.577002541757441]);
...
leg.Position = [0.6554    0.6918    0.0991    0.0618];
...
p1 = plot(tr_ekemr(sig_np_ekemr==1)*1e2, S_m(sig_np_ekemr==1), 's', ...
    'markerfacecolor', rgb('Gray'), 'markeredgecolor', rgb('Gray'),...
    'markersize', 5);
b4.Color = rgb('Gray');
b5.FaceColor = rgb('Gray');
set(ax3, 'Box', 'on', 'TickDir', 'out', 'TickLength', [0.03 0.03], ...
...
annotation(fig0, 'line', [0.667079949144191 0.616241713850074],   ...
    [0.072013088486425 0.072013088486425]);
...
set(gca, 'Box', 'on', 'TickDir', 'out', 'TickLength', [0.03 0.03], ...
...
ax4.Position(2) = ax4.Position(2)+0.02;
ax5.Position(1) = ax2.Position(1);
ax5.Position(3) = ax2.Position(3);
ax5.Position(2) = ax5.Position(2)+0.02;
...
ax6.Position(2) = ax5.Position(2);
ax6.Position(1) = ax3.Position(1);
ax6.Position(3) = ax3.Position(3);
ax6.Position(4) = ax5.Position(4);
...
% print('-dpdf', ['./fig7_trends_SSH_eke_emf.pdf'], '-r500');
