function [gadcp all_lmg] = grid_adcp_dates(gsize,data,itype,varargin);
...
% and the time period (dates).


if length(varargin)==0
   load ([cd '/drake_mean_2021/data/all_drake/', 'DPshort'] , 'theta');
   overlapping = 0;
   theta0 = theta;
...
load([cd '/drake_mean_2021/data/all_drake/lmg_' itype]);
...
if length(varargin) == 5;% Finds data between specific dates
    ...
    ind_dates = find(all_lmg.t >= datenum(dates(1)) & ...
        all_lmg.t <= datenum(dates(2)));
    
    
    all_lmg.uv = all_lmg.uv(:,ind_dates);
    all_lmg.uvupd = all_lmg.uvupd(ind_dates);
    all_lmg.t = all_lmg.t(ind_dates);
    all_lmg.lon = all_lmg.lon(ind_dates);
    all_lmg.lat = all_lmg.lat(ind_dates);   
    
    tide.u = tide.u(ind_dates);
    tide.v = tide.v(ind_dates);
...
ur = real(complex(ur,vr)*exp(-sqrt(-1)*theta/180*pi));
vr = imag(complex(ur,vr)*exp(-sqrt(-1)*theta/180*pi));

all_lmg.uv = all_lmg.uv*exp(-sqrt(-1)*theta/180*pi);
...
[gpos, xr, yr] = makegrid2...
    (all_lmg.lat, all_lmg.lon-360,gsize,0,theta,lat0,lon0); % compute grid positions
...
[gpos3, xr3, yr3] = makegrid2...
    (lat_d,lon_d-360,gsize,0,theta,lat0,lon0); % compute grid positions
