...
% geostrophic, total and reference components. Estimates mean transport per
% distance bin and trends after removing the seasonal cycle (annual +
% semiannual) frequencies. Trends are calculated using least squares and
% significance test is done by the modified Mann-Kendall test. 
%
% 2023/06/25 - Created by Manuel O. Gutierrez-Villanueva
% 2023/08/28 - Now saved as fig4
...
% Saves data in new variables
tot_xbt = tot.transp(:, ind_tot2);
tot_xbt_lat = tot.lat(:, ind_tot2);
tot_xbt_lon = tot.lon(:, ind_tot2);
geos_xbt = geos.transp(:, ind_geo2);
geos_xbt_lat = geos.lat_u(:, ind_geo2);
geos_xbt_lon = geos.lon_u(:, ind_geo2);
geos_xbt_lat(isnan(geos_xbt)) = NaN;
geos_xbt_lon(isnan(geos_xbt)) = NaN;

latmax_geos_xbt = nanmin(geos_xbt_lat);
latmax_tot_xbt = nanmin(tot_xbt_lat);

for ii = 1:length(ind_tot2);
    iind = find(tot_xbt_lat(:, ii) < latmax_geos_xbt(ii))
    tot_xbt(iind, ii) = NaN;
    tot_xbt_lat(iind, ii) = NaN;
    tot_xbt_lon(iind, ii) = NaN;
end
tot_xbt = tot_xbt*1e-6;

% Calculates trends
[tr_tot, ci_tot, ~, tr_np_tot sig_np_tot] = calctrends(tot.transp, ...
    [tot.time-datenum(1996, 1, 1)]/365.25, alpha, op_season);
[tr_tot2, ci_tot2, ~, tr_np_tot2 sig_np_tot2] = calctrends(totxbt, ...
...
hold on;
[hl2 hp2] = boundedline(nanmean(tot_xbt, 2), tot.dist, ...
    nanstd(tot_xbt, 0 , 2)./sqrt(sum(~isnan(tot_xbt), 2)), '-',...
    'alpha', 'Orientation', 'horiz');
hl2.MarkerSize = 5;
hl2.Color = rgb('Green');
...
leg = legend([hl1; hl2], [num2str(nansum(nanmean(tot.transp*1e-6, 2)),'%2.2f')...
    ...
    ' Sv'], [num2str(nansum(nanmean(tot_xbt, 2)),'%2.2f')...
    char(177) num2str(nanstd(nansum(tot_xbt, 1))/sqrt(size(tot_xbt, 2)),'%2.2f') ...
...
hl2.MarkerSize = 10;
[hl2 hp2] = boundedline(tr_tot2, tot.dist, ci_tot2, '-b',...
    'alpha', 'Orientation', 'horiz');
pp2 = plot(tr_tot2(sig_np_tot2==1), tot.dist(sig_np_tot2==1), 's', ...
...
leg3 = legend([hl4], [num2str(nansum(nanmean(ref.transp*1e-6, 2)),'%2.2f')...
    ...
    ' Sv'], 'fontsize', 5, 'fontname', 'SansSerif');
txt1 = text(0.5, 550, ['Ekman = ' num2str(nanmean(ref.eknettransp*1e-6), '%2.2f') char(177) ...
     num2str(nanstd(ref.eknettransp*1e-6)/sqrt(size(ref.transp, 2)),'%2.2f') ' Sv'], ...
    'fontsize', 5, 'fontname', 'SansSerif');
r1.Color = rgb('Blue'); 
r2.Color = r1.Color; 
...
annotation(fig0, 'textbox', [ha(15).Position(1)-0.03 sum(ha(15).Position([2,4]))+0.01 ...
    ...
    'EdgeColor', 'none', 'fontweight', 'bold');
print('-dpdf', './FiguresPaper/fig4_transpdist_mean.pdf', '-r500');
