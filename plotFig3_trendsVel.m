% Code for plotting Figure 3 - Trends in the cross-transect velocity as
% function of depth and distance across the Passage. Velocity trends are
% estimated after removing the seasonal cycle (annual+semiannual harmonics)
% using least-squares to fit a line. Statistical significance is calculated
% with the Modified Mann-Kendall test.
%
...
% corrected for interpolation issues while using griddata.
...
% geosbci_760.dist_u = distubci + 12.5; 
clear geosbaroc 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% geosref_760.u = geosref_760.u(:, 1:end, :);
% distref = 0.5*(geosref_780.dist(1:end-1, :) + geosref_780.dist(2:end, :));
% distref = distref - distref(1, :);
% geosbci_760.dist_u = distref; 
...
% axes(ax(4));
% ax4Pos = ax(4).Position;
% patch([820 745 745 820], [ones(1, 2)*-2.4 ones(1, 2)*2.4], ...
%     rgb('Goldenrod'), 'edgecolor', 'none', 'facealpha', 0.2); hold on;
% patch([300 500 500 300], [ones(1, 2)*-2.4 ones(1, 2)*2.4], ...
%     rgb('Goldenrod'), 'edgecolor', 'none', 'facealpha', 0.2);
% patch([35 135 135 35], [ones(1, 2)*-2.4 ones(1, 2)*2.4], ...
%     rgb('Goldenrod'), 'edgecolor', 'none', 'facealpha', 0.2);
% axis(ax(4), [0 837.50 -2.4 2.4])
% [a4 aa4] = boundedline(geosbci_760.dist_u(1:72, 1)', trend_ref760(1:72)'*1e2,  ...
%     abs(confint_ref760(1:72)')*1e2, '-k', 'alpha');
% p4 = plot(geosbci_760.dist_u(sigtr_np_ref760 == 1, 1)', ...
%     trend_np_ref760(sigtr_np_ref760 == 1)'*1e2, 'sk', ...
%     'markerfacecolor', rgb('Blue'), 'markeredgecolor', rgb('Blue'));
% a4.Color = rgb('Blue'); aa4.FaceColor = a4.Color;
% plot([ax(4).XLim], [0 0], '-', 'color', [0.5 0.5 0.5]);
% % axis ij; axis(ha(6), [-0.089 0.089 ha(1).YLim]);
% set(ax(4), 'TickLength', [0.020 0.020], 'box', 'on', 'TickDir', 'out', ...
%     'XTick', ax(1).XTick, 'XTickLabel', num2str(ax(1).XTick(:)), ...
%     'YTick', [-2.0:2.0]', 'YTickLabel', num2str([-2.0:2.0]'), ...
%     'fontsize', 7, 'fontname', 'SansSerif');
% ylabel('Trend [cm s^{-1} year^{-1}]', 'fontsize', 7, 'fontname', 'SansSerif');
% text(20, -1.6, 'Reference velocity u_{ref}', 'fontsize', 7, ...
%     'fontname', 'SansSerif', 'fontweight', 'bold');
% text(60, 1.5, 'SAF', 'fontsize', 7, 'fontname', 'SansSerif');
% text(380, 1.5, 'PF', 'fontsize', 7, 'fontname', 'SansSerif');
% text(780, -0.99, 'SACCF', 'fontsize', 7, 'fontname', 'SansSerif', ...
%     'Rotation', 90);
% xlabel('Distance from North [km]', 'fontsize', 7, 'fontname', 'SansSerif')
...
annotation(fig1, 'textbox', [0 sum(ax(3).Position([2,4]))+0.025 ...
    ...
    'EdgeColor', 'none', 'fontweight', 'bold');
% annotation(fig1, 'textbox', [0 sum(ax(4).Position([2,4]))+0.025 ...
