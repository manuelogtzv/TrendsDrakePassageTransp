% Code for calculating trends for total, geostrophic and reference transport using
% different number of transects to test sensitivity. Trends are estimated using least-squares after 
...
cont1 = [-20.0:2:20.0];%vel contours
cmap1 = getPyPlot_cMap('seismic', length(cont1)-1, [], pycmd);
cont2 = [-3.0:0.5:3];%vel contours
cmap2 = getPyPlot_cMap('seismic', length(cont2)-1, [], pycmd);
cont3 = [-10:10];%vel contours
cmap3 = getPyPlot_cMap('seismic', length(cont3)-1, [], pycmd);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Loads os38 data %%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Loads transport
load('TransportDP_adcp_os38nbmaxz780_2.mat'); %os38nb
tot = transpDP;
...
load('TransportDP_baroc_noaverg.mat'); %baroclinic
geo = barocDP; 
...
errorbar(3, tr_ref*1e-1, conf_ref*1e-1, 'b', 'linewidth', 1); 
hold on;
...
print(gcf,'./FiguresPaper/figSI_trendtottransp.png','-dpng','-r1000');
